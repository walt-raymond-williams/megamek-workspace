...
- `Confirmed from source`: mothballing a unit does not clear its explicit transport assignment. `Unit#completeMothball()` sets the unit mothballed and removes the unit from campaign transporter maps only if the mothballed unit itself is a transport; the source comment says transport assignments should not be cleared. A mothballed unit already assigned to a DropShip therefore remains in the transport's transported-unit set and still reserves bay capacity when MekHQ recalculates transporter capacity through `AbstractTransportedUnitsSummary#loadTransportedEntities()`. Separately, `CargoStatistics#getCargoTonnage(..., mothballed=true)` counts every mothballed unit's weight as mothballed cargo tonnage for cargo/cost summaries, with a FIXME noting that the place-units-in-bays logic is temporarily disabled pending DropShip assignment work. Practical result: do not assume the UI/cost summaries have moved the unit out of its assigned Mek bay; explicitly unassign transport if you want that bay freed.
