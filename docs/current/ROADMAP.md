...
- Issue: `#94`
- Owner: `Mixed`
- Goal: Turn MEK-RPG's live play API gap request into bounded producer-side MekHQ local API work, prioritizing mission-planning facts, source-owned ledgers, logistics capacity, personnel education/turnover, and verification without accepting unbounded or unsourceable endpoint requests.
...
- Child issues:
  ...
  - `#109`: Investigate education enrollment and graduation API gap. Completed locally in MekHQ source commit `b9f42710a9`; archived handoff: `docs/handoffs/archive/investigate-education-school-api-gap.md`. Source push is blocked by upstream `MegaMek/mekhq` 403.
...
- Recommended sequence: Education issue `#109` is implemented locally. Next complete design issue `#95` if endpoint-wide contract decisions remain unsettled, implement scenario intel `#96`, run bounded ledger issue `#100` alongside or after activity-history design `#58`, and run inventory/transport `#97` before broader personnel turnover `#98` unless source audit changes the order. Use docs/fixtures/smoke issue `#99` after the first implementation slices land.
- Open questions: Does MekHQ store structured school/enrollment/graduation records or only status/log/report facts for student personnel? Should scenario intel become a dedicated endpoint or stay in `/campaign/pending-deployments` and scenario state DTOs? Which salvage/reputation/XP histories are durable source data rather than report text? Does MekHQ track exact transport/recovery capacity and HR/admin pressure, or should V1 return partial facts with explicit unsupported entries?
...
- Handoff notes: This is an epic, not a direct implementation task. Active tracking doc: `docs/current/PLANETARY_INFORMATION_API_TRACKING.md`. Source audit `#84` is complete in `docs/current/MEK_RPG_LIVE_MEKHQ_PLANETARY_API_SOURCE_AUDIT.md`; design issue `#85` is complete in `docs/current/MEK_RPG_LIVE_MEKHQ_PLANETARY_API_DESIGN.md`; active handoffs remain for child issues `#86` and `#87`.
...
- Child issues:
  - `#84`: Audit MekHQ Navigation tab planetary data sources. Completed on `2026-07-03`; source audit: `docs/current/MEK_RPG_LIVE_MEKHQ_PLANETARY_API_SOURCE_AUDIT.md`; archived handoff: `docs/handoffs/archive/audit-navigation-planetary-data-sources.md`.
  - `#85`: Design read-only MekHQ planetary information API. Completed on `2026-07-04`; design note: `docs/current/MEK_RPG_LIVE_MEKHQ_PLANETARY_API_DESIGN.md`; archived handoff: `docs/handoffs/archive/design-planetary-information-api.md`.
...
- Recommended sequence: Implement `#86` next using the issue `#85` design. Use `#87` after implementation exists, or earlier only to draft fixture/smoke expectations.
- Open questions: Which planet/system should become the canonical sanitized fixture for MEK-RPG adapter tests? Should a later search/list endpoint expose planetary lookup candidates before detail requests? Should arbitrary historical/future `asOfDate` support be added after academies, campaign overlays, and user data behavior are source-confirmed?
...
- Child issues:
  ...
  - `#79`: Smoke test contract accept prompts and MekHQ UI refresh. Blocked on `2026-07-01` until the user is available for live/full UI testing; active handoff: `docs/handoffs/active/smoke-test-contract-accept-ui-refresh.md`.
...
- Recommended sequence: The implemented guarded command slice now covers status-note, personnel status, personnel fatigue, unit-market purchase, contract accept, and pilot/TO&E read selectors. MEK-RPG live play exposed a command gap for pilot assignment and TO&E edits; focused epic `#70` now has source audit `#71`, command design `#72`, and selector issue `#73` complete. Block live/full-UI smoke tickets until the user is available. While blocked, continue with source/doc/API-design issues that do not require live user testing. Issue `#79` remains the high-risk contract prompt/UI-refresh smoke gap; issues `#51`, `#53`, and `#54` still need live disposable-campaign smoke testing when representative copied campaigns and the user are available.
...
- Handoff notes: Active handoff: `docs/handoffs/active/smoke-test-contract-accept-ui-refresh.md`. Use copied/disposable campaign saves only. Keep `saveAfterSuccess=false` unless intentionally testing a disposable save path. Blocked on `2026-07-01` because the user directed that anything requiring user/full UI testing should wait until the user is available. An autonomous attempt launched source-built MekHQ and created disposable copy `analysis/tmp/issue-79/issue-79-contract-smoke-demo.cpnx.gz`, but no campaign was loaded and no mutation was run.
- Dependencies: Completed contract accept implementation issue `#55`; source-built MekHQ launched with `-Dmekhq.controlApi.enabled=true`; copied/disposable campaign with at least one selectable contract-market offer; user availability for live/full UI validation.
