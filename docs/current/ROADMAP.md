...
- Child issues:
  ...
  - `#12`: Implement robust tabletop battle-record MUL generator. Narrowed by issue `#11`: only proceed as a workspace installed-jar helper if generated battle-record MULs are needed before built-in/manual workflow is enough.
  - `#13`: Verify and document tabletop result entry workflow for MekHQ.
- Recommended sequence: Source workflow confirmation `#8`, salvage research `#7`, input schema `#9`, generated-MUL study `#22`, and strategy issue `#11` are complete. Issue `#10` has validated native battle-record MUL writer/parser round trip but still needs a live MekHQ Resolve Manually import pass; carry the checklist from `GENERATED_BATTLE_RECORD_MUL_STUDY.md` into that user-operated pass. Next choose whether to narrow/execute `#12` as a workspace helper or skip it and move to `#13` built-in/manual workflow documentation.
...
- Open questions: Is built-in/manual entry sufficient for the first real campaign once user task `#23` creates a realistic campaign? Should issue `#12` proceed as a workspace helper or be closed as unnecessary for the first campaign? Which exact live UI checks should issue `#13` preserve?
