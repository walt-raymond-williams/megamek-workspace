...
- Recommended sequence: Harden `/campaign/summary` and `/campaign/commands` through `#64` using the issue `#63` timing diagnostics, fix narrowed state behavior through `#65`, add the purpose-built pending-deployment read path through `#66`, and use `#67` to lock in regression and live-smoke coverage.
