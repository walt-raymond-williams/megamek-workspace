...
- Child issues:
  ...
  - `#22`: Study generated battle-record MUL from MekHQ shakedown. Created after the user saved a live MegaMek result MUL at `external/installs/MekHQ-0.51.00/The Learning Ropes.mul`; active handoff: `docs/handoffs/active/study-generated-battle-record-mul.md`.
...
- Recommended sequence: Source workflow confirmation `#8`, salvage research `#7`, and input schema `#9` are complete. Issue `#10` has validated native battle-record MUL writer/parser round trip but still needs a live MekHQ Resolve Manually import pass. Issue `#22` should study the newly generated live battle-record MUL before or alongside the user's next manual import attempt. If the UI pass remains unavailable, proceed to `#11` with that caveat, decide whether implementation `#12` is still needed, and finish with UI/manual documentation `#13`.
...
- Recommended sequence: `#20` defined the physical miniature roster model, `#18` documented the fixed OPFOR setup-MUL pool path, and `#19` decided not to build custom RATs yet. `#17` remains blocked on live UI, with the user-owned validation task tracked as `#21`. Next roster-control progress requires either that user-operated MekHQ UI pass or the user's confirmed physical miniature list.
  - Child issues:
