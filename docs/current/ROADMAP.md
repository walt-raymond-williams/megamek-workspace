...
- Recommended sequence: `#20` defined the physical miniature roster model, `#18` documented the fixed OPFOR setup-MUL pool path, and `#19` decided not to build custom RATs yet. `#17` remains blocked on live UI. Next roster-control progress requires either a user-operated MekHQ UI pass or the user's confirmed physical miniature list.
  - Child issues:
