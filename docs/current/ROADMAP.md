...
- Child issues:
  ...
  - `#109`: Investigate education enrollment and graduation API gap. Active handoff: `docs/handoffs/active/investigate-education-school-api-gap.md`.
...
- Recommended sequence: Treat education issue `#109` as the next P1 user-blocking slice because Sharpe's Strikers roster management needs it immediately. Then complete design issue `#95` if endpoint-wide contract decisions remain unsettled, implement scenario intel `#96`, run bounded ledger issue `#100` alongside or after activity-history design `#58`, and run inventory/transport `#97` before broader personnel turnover `#98` unless source audit changes the order. Use docs/fixtures/smoke issue `#99` after the first implementation slices land.
