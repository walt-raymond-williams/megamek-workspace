...
- Recommended sequence: The implemented guarded command slice now covers status-note, personnel status, personnel fatigue, unit-market purchase, and contract accept. MEK-RPG live play exposed a command gap for pilot assignment and TO&E edits; focused epic `#70` now has source audit `#71` complete and should continue through design `#72`, selectors `#73`, and guarded implementation issues `#74` through `#77` before deciding whether epic `#44` is PR-ready. Issues `#51`, `#53`, `#54`, and `#55` still need live disposable-campaign smoke testing when a source-built MekHQ instance is available.
- Open questions: Which medical/prosthetic state is available under the user's active MekHQ options? Which contract prompt choices should V1 support beyond accepting known confirmations, acknowledging informational prompts, and declining optional travel/rental automation? For TO&E and pilot assignment, issue `#71` found that role/eligibility validation is mostly embedded in Swing menus, so issue `#72` must decide whether implementation extracts shared non-dialog validators or starts with a narrow command service.
...
- Recommended sequence: Source audit `#71` is complete. Proceed with API design `#72`, then read selectors `#73`. Implement pilot commands `#74` and TO&E commands `#75` only after selectors and the source validator/service extraction path are stable. Treat batch issue `#76` as optional and dependent on the individual command designs. Run coverage/smoke issue `#77` after the mutating slices exist.
- Open questions: Should V1 allow assignment to mothballed units or require `Unit#isAvailable()` exactly? What conservative force-name validation should create/rename use? Should role/eligibility checks be extracted from Swing menus into a reusable service before implementation, or mirrored in a command service with focused regression tests first?
