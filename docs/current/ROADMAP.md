...
- Recommended sequence: The implemented guarded command slice now covers status-note, personnel status, personnel fatigue, unit-market purchase, and contract accept. MEK-RPG live play exposed a command gap for pilot assignment and TO&E edits; focused epic `#70` now has source audit `#71` and command design `#72` complete. Continue with selectors `#73`, then guarded implementation issues `#74` and `#75`, then fixture/smoke issue `#77`; keep batch issue `#76` optional until individual commands are stable. Issues `#51`, `#53`, `#54`, and `#55` still need live disposable-campaign smoke testing when a source-built MekHQ instance is available.
