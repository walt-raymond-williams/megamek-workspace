...
- Recommended sequence: `#20` defined the physical miniature roster model, and `#18` documented the fixed OPFOR setup-MUL pool path. `#17` remains blocked on live UI. Next decide whether custom RATs are worth the extra complexity in `#19`, using fixed MUL pools as the baseline.
  - Child issues:
