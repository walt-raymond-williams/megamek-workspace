...
- `2026-06-22`: User-assisted live test of the issue `#35` prototype succeeded. The source-built MekHQ app launched with `mekhq.controlApi.enabled=true`; `/status` saw loaded campaign `The Learning Ropes`; `POST /advance-day` advanced exactly one day from `3025-04-08` to `3025-04-09` with `saveAfterSuccess=false`, no visible dialogs, and no save attempt.
